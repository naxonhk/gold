  ...
  return (
    <div>
        ...
        <div className="modal-overlay" onClick={() => setShowAuthModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAuth}>
              ...
              {authError && <p style={{ color: 'var(--danger)', marginBottom: '16px', fontSize: '14px' }}>{authError}</p>}
            ...
            <div style={{ marginTop: '20px', textAlign: 'center' }}>
              ...
              <button onClick={handleGoogleLogin} className="btn btn-secondary" style={{ width: '100%', background: '#fff', color: '#333' }}>
                使用 Google 登入
              </button>
            </div>
            ...
            <p style={{ textAlign: 'center', marginTop: '20px', color: 'var(--text-muted)', fontSize: '14px' }}>
              {isLogin ? '還沒有賬戶？' : '已有賬戶？'}
              <button onClick={() => setIsLogin(!isLogin)} style={{ background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', marginLeft: '8px', fontWeight: '600' }}>
                {isLogin ? '立即註冊' : '立即登入'}
              </button>
            </p>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setShowSettings(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={preferredUnit} onChange={e => handleSavePreferences(e.target.value)}>
          ...
          <div className="dashboard">
            <div className="dashboard-header">
              ...
              <div className="dashboard-controls">
                <select className="form-select" style={{ width: 'auto', minWidth: '120px' }} value={selectedSource} onChange={e => setSelectedSource(e.target.value)}>
                  <option value="chowtaifook">周大福</option>
                  <option value="chowsangsang">周生生</option>
                </select>
                <button onClick={fetchPrices} className="btn btn-secondary btn-sm" disabled={priceLoading}>
                  {priceLoading ? '🔄' : '🔄 刷新'}
                </button>
              </div>
            </div>
            ...
            <div className="stats-grid">
              ...
              <div className="stat-card">
                ...
                <div className="stat-value" style={{ color: 'var(--danger)' }}>HK$ {totalBuy.toLocaleString()}</div>
                <div className="stat-change">{buyRecords.length} 筆</div>
              </div>
              
              <div className="stat-card">
                <div className="stat-label">總回收</div>
                <div className="stat-value" style={{ color: 'var(--success)' }}>HK$ {totalSell.toLocaleString()}</div>
            ...
            <div className="card" style={{ marginBottom: '24px' }}>
              ...
              {selectedSource === 'chowtaifook' ? (
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ padding: '14px', background: 'rgba(255,215,0,0.08)', borderRadius: '10px' }}>
                    <div style={{ fontWeight: '700', marginBottom: '10px', color: 'var(--primary)' }}>💎 999.9 黃金</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>售價</div>
                        <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--success)' }}>HK$ {prices.chowtaifook.gold999.sell.toLocaleString()}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>回收價</div>
                        <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--danger)' }}>HK$ {prices.chowtaifook.gold999.buy.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                  
                  <div style={{ padding: '14px', background: 'rgba(255,215,0,0.05)', borderRadius: '10px' }}>
                    <div style={{ fontWeight: '700', marginBottom: '10px', color: 'var(--primary)' }}>🪙 黃金粒</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>售價</div>
                        <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--success)' }}>HK$ {prices.chowtaifook.goldPellet.sell.toLocaleString()}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>回收價</div>
                        <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--danger)' }}>HK$ {prices.chowtaifook.goldPellet.buy.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ padding: '14px', background: 'rgba(255,215,0,0.08)', borderRadius: '10px' }}>
                    <div style={{ fontWeight: '700', marginBottom: '10px', color: 'var(--primary)' }}>💍 黃金飾品</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>售價</div><div style={{ fontWeight: '800', color: 'var(--success)' }}>HK$ {prices.chowsangsang.goldOrnaments.sell.toLocaleString()}</div></div>
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>兌換</div><div style={{ fontWeight: '800', color: '#60a5fa' }}>HK$ {prices.chowsangsang.goldOrnaments.exchange.toLocaleString()}</div></div>
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>回收</div><div style={{ fontWeight: '800', color: 'var(--danger)' }}>HK$ {prices.chowsangsang.goldOrnaments.buy.toLocaleString()}</div></div>
                    </div>
                  </div>
                  
                  <div style={{ padding: '14px', background: 'rgba(255,215,0,0.05)', borderRadius: '10px' }}>
                    <div style={{ fontWeight: '700', marginBottom: '10px', color: 'var(--primary)' }}>📐 金條</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>售價</div><div style={{ fontWeight: '800', color: 'var(--success)' }}>HK$ {prices.chowsangsang.goldIngot.sell.toLocaleString()}</div></div>
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>回收</div><div style={{ fontWeight: '800', color: 'var(--danger)' }}>HK$ {prices.chowsangsang.goldIngot.buy.toLocaleString()}</div></div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="card calculator-section">
              ...
              <div className="calc-form">
                <div className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  <select className="form-select" value={calcGoldType} onChange={e => setCalcGoldType(e.target.value)}>
                    {selectedSource === 'chowtaifook' ? (
                      <><option value="gold999">999.9 黃金</option><option value="goldPellet">黃金粒</option></>
                    ) : (
                      <><option value="goldOrnaments">黃金飾品</option><option value="goldIngot">金條</option><option value="goldBars">金粒</option></>
                    )}
                  </select>
                </div>
                ...
                <div className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  <input type="number" className="form-input" value={calcWeight} onChange={e => setCalcWeight(e.target.value)} placeholder="輸入重量" />
                </div>
                
                <div className="form-group" style={{ marginBottom: 0 }}>
                  <label className="form-label">單位</label>
                  <select className="form-select" value={calcWeightUnit} onChange={e => setCalcWeightUnit(e.target.value)}>
                ...
                <button onClick={handleCalculate} className="btn btn-primary" style={{ alignSelf: 'end' }}>計算</button>
              </div>
              
              {calcResult && (
                <div className="calc-result">
                  <div className="calc-result-label">{calcType === 'sell' ? '售價' : '回收價'}：HK$ {calcResult.pricePerTael.toLocaleString()}/両</div>
                  <div className="calc-result-value">HK$ {calcResult.total.toLocaleString()}</div>
                  <div className="calc-result-gram">{calcResult.weightInTael.toFixed(3)} 両 | {calcResult.weightInGram.toFixed(2)} 克</div>
                </div>
              )}
            </div>
            ...
            <div className="records-section">
              <div className="records-header">
                ...
                <button onClick={() => setShowAddForm(!showAddForm)} className="btn btn-primary btn-sm">+ 新增</button>
              </div>
                  ...
                  <form onSubmit={handleAddRecord}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        ...
                        <input type="number" className="form-input" value={newRecord.weight} onChange={e => setNewRecord({...newRecord, weight: e.target.value})} placeholder="0" step="0.01" required />
                      </div>
                      
                      <div className="form-group" style={{ marginBottom: 0, gridColumn: 'span 2' }}>
                        <label className="form-label">單價（HK$/両）</label>
                        <input type="number" className="form-input" value={newRecord.price} onChange={e => setNewRecord({...newRecord, price: e.target.value})} placeholder={prices[selectedSource]?.gold999?.sell?.toString() || ''} required />
                      </div>
                      
                      <div className="form-group" style={{ marginBottom: 0, gridColumn: 'span 2' }}>
                        <label className="form-label">備註（選填）</label>
                        <input type="text" className="form-input" value={newRecord.note} onChange={e => setNewRecord({...newRecord, note: e.target.value})} placeholder="新增備註..." />
                      </div>
                    </div>
              ...
              <div className="records-filters">
                <button className={`filter-btn ${filter === 'all' ? 'active' : ''}`} onClick={() => setFilter('all')}>全部 ({records.length})</button>
                <button className={`filter-btn ${filter === 'buy' ? 'active' : ''}`} onClick={() => setFilter('buy')}>買入 ({buyRecords.length})</button>
                <button className={`filter-btn ${filter === 'sell' ? 'active' : ''}`} onClick={() => setFilter('sell')}>賣出 ({sellRecords.length})</button>
              </div>
                  ...
                  {filteredRecords.map(record => (
                    <div key={record.id} className="record-item">
                      <div className="record-info">
                        <span className={`record-type ${record.type}`}>{record.type === 'buy' ? '買入' : record.type === 'sell' ? '賣出' : '送出'}</span>
                        <div className="record-details">
                          <span>📊 {record.weight} 両</span>
                          <span>💵 HK$ {record.price?.toLocaleString()}/両</span>
                          <span>🏪 {record.source === 'chowtaifook' ? '周大福' : '周生生'}</span>
                        </div>
                        {record.note && <div className="record-details" style={{ marginTop: '4px', fontSize: '13px' }}>📝 {record.note}</div>}
                      </div>
                      <div className="record-value">
                        <div className="record-total">HK$ {record.total?.toLocaleString()}</div>
                        <div className="record-date">{record.createdAt?.toLocaleDateString?.('zh-HK') || new Date().toLocaleDateString('zh-HK')}</div>
                        <button onClick={() => handleDeleteRecord(record.id)} className="btn btn-danger btn-sm" style={{ marginTop: '8px' }}>刪除</button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <nav className="mobile-nav">
        <Link href="/" className="mobile-nav-item"><span>🏠</span><span>首頁</span></Link>
        <Link href="/dashboard" className="mobile-nav-item active"><span>💰</span><span>資產</span></Link>
      </nav>
    </div>
  );
}
